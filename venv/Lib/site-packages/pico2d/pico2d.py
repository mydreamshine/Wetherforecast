...
def open_canvas(w=int(800), h=int(600), sync=False, full=False, caption = 'Pico2D Canvas'):
    ...
    Caption = (caption + '(' + str(w) + 'x' + str(h) + ')' + ' 1000.0 FPS').encode('UTF-8')
    if full:
...
def resize_canvas(w, h):
    ...
    caption = (Wnd_Title + '(' + str(w) + 'x' + str(h) + ')' + ' 1000.0 FPS').encode('UTF-8')
...
def print_fps():
    ...
    cur_time += dt
    dt = max(dt, 0.0001)
    caption = (Wnd_Title + '(' + str(canvas_width) + 'x' + str(canvas_height) + ')' + ' %4.2f FPS' % (1.0/dt)).encode('UTF-8')
    SDL_SetWindowTitle(window, caption)
...
class Font:
    ...
    def getpixelSize(self, str):
        surface = TTF_RenderText_Blended(self.font, str.encode('utf-8'), SDL_Color(0, 0, 0))
        texture = SDL_CreateTextureFromSurface(renderer, surface)
        SDL_FreeSurface(surface)
        image = Image(texture)
        w, h = image.w, image.h
    ...
    def getpixelSize_unicode(self, str):
        surface = TTF_RenderUNICODE_Blended(self.font, ctypes.cast(str.encode('utf-16'), ctypes.POINTER(ctypes.c_uint16)), SDL_Color(0, 0, 0))
        texture = SDL_CreateTextureFromSurface(renderer, surface)
        SDL_FreeSurface(surface)
        image = Image(texture)
        w, h = image.w, image.h
